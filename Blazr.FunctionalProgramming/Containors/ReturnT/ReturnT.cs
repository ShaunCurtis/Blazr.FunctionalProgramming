...
public sealed record Return<T>
{
    ...
    public static Return<T> Read(T? value)
        => value is null
        ? new(new ReturnTException("T was null."))
        : new(value);

    public static Return<T> Read(T? value, string errorMessage) =>
        value is null
            ? new(new ReturnTException(errorMessage))
            : new(value);
    ...
    public static Return<T> Failure(string message) => new(new ReturnTException(message));
}
